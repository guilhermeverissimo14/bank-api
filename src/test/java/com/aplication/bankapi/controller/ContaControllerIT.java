...
class ContaControllerIT {
        ...
        @Test
        void deveSacarEConsultarSaldo() throws Exception {
                Cliente cliente = clienteRepository.save(Cliente.builder()
                                .nome("Ana")
                                .email("anateste@teste.com")
                                .senha(passwordEncoder.encode("senha123"))
                                .build());
                ...
                mockMvc.perform(post("/contas/{id}/sacar", conta.getId())
                                .with(jwt())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(new ValorRequest(new BigDecimal("50.00")))))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.saldo").value(150.00));
